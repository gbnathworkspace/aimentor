...
| 2 | **Missing env** | Frontend | `mentorman-web` had no `.env` — Clerk publishable key missing, app threw on startup |
...
| 5 | **Incomplete backend `.env`** | Backend | Missing `CLERK_ISSUER` and `CORS_ORIGINS` — auth and CORS would've silently failed |
| 6 | **Double blank screen** | Frontend | `if (!profileLoaded) return null` caused a second invisible wait after Clerk loaded |
