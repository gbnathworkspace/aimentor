...
- [x] 1. Add the SummaryBlock storage shape
  - [x] 1.1 Add `summaryBlocks: list[SummaryBlock]` field to the topic document model
  ...
  - [x] 1.2 Write model/schema tests for the new field
    - Empty default, round-trip serialization
    - _Requirements: 2.2_

- [x] 2. Implement session boundary detection
  - [x] 2.1 Add `session_boundary.py` with `check_and_close_on_new_message`
  ...
  - [x] 2.3 Add `idle_sweep()` and a startup-registered background loop
...
- [x] 3. Implement session summarization
  - [x] 3.1 Add `session_summarizer.py` with `close_session(topic_id, user_id, upto_timestamp)`
  ...
  - [x] 3.3 Write unit tests for `close_session`
...
- [x] 4. Implement bounded-storage merging
  - [x] 4.1 Add `_call_merge_two_blocks_llm(text_a, text_b) -> str`
    - New prompt variant combining two already-summarized blocks, respecting
      `BLOCK_WORD_FLOOR`
    - _Requirements: 3.1, 3.4_

  - [x] 4.2 Add `enforce_word_cap(blocks) -> list[dict]`
  ...
  - [x] 4.3 Wire `enforce_word_cap` into `close_session` after appending the fresh block
    - _Requirements: 3.1_

  - [x] 4.4 Write unit tests for `enforce_word_cap`
    - Under cap → no merge
  ...
  - [x] 4.5 Write property test for no-double-summarization (Property 1)
...
- [x] 5. Checkpoint - Ensure session_boundary + session_summarizer tests pass

- [x] 6. Integrate with ContextAssembler
  - [x] 6.1 Add `_format_summary_blocks` alongside `_format_episodes` in `prompt_store.py`
    - Joins `topic.summaryBlocks` in `createdAt` order, no 300-char truncation
    - _Requirements: 7.1, 7.2_

  - [x] 6.2 Exempt topic's own SummaryBlocks from `_format_episodes`'s 300-char cap
    - Cross-topic `_recent_episodes` entries keep the existing truncation
    - _Requirements: 7.1_

  - [x] 6.3 Write regression tests for context assembly
    - A 500-word SummaryBlock is not truncated when injected
    - Cross-topic episodes are still truncated at 300 chars as before
    - _Requirements: 7.1, 7.2_

- [x] 7. Confirm independence from existing systems
  - [x] 7.1 Verify `CompactionService`'s RollingSummary merge is untouched by this spec
    - Existing rolling-topic-summary test suite passes unmodified
    - _Requirements: 4.1, 4.4_

  - [x] 7.2 Verify `taughtConcepts` behavior is unchanged apart from its new trigger
  ...
  - [x] 7.3 Verify no reads/writes to `sessions_col`, `SessionManager`, or
    `SessionSaveHandler` were introduced
    - _Requirements: 6.1_

- [x] 8. Final checkpoint - Ensure full backend test suite passes
