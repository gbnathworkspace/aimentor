...
export interface SkillGraphNodeDocument
  extends Omit<SkillGraphNode, 'createdAt' | 'updatedAt' | 'last_studied'>,
    Document {
  createdAt: Date
  updatedAt: Date
  last_studied?: Date
...
const SkillGraphNodeMongoSchema = new Schema<SkillGraphNodeDocument>(
  {
    ...
    category: {
      type: String,
      required: true,
      enum: ['DSA', 'Cloud', 'Core', 'System Design'],
    },
    required_level: {
      type: String,
      required: true,
  ...
  {
    timestamps: true,
    collection: 'skill_graph_nodes',
  }
)

// Compound index: one node per (userId, topic)
